...
class Mollom
  ...
  def image_captcha info = {}
    result = send_command('mollom.getImageCaptcha', info)
    #puts "gem mollom-0.1.4 image captcha result #{result.inspect}"
    return result
  end
  ...
  def server_list refresh = false
    #puts "gem mollom server_list refresh #{refresh.inspect}"
    return @server_list if @server_list && refresh
    #puts "gem mollom server_list updating server list from remote"
    STATIC_SERVER_LIST.each do |static_server|
      #puts "gem mollom  Requesting server list from #{static_server.inspect}"
      @server_list = get_server_list_from(static_server)
      #puts "gem mollom  Requesting server list from #{static_server.inspect} @server_list #{@server_list.inspect}"
